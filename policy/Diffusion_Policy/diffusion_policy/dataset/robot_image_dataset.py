...
class RobotImageDataset(BaseImageDataset):
    def __init__(
        ...
        self.replay_buffer = ReplayBuffer.copy_from_path(
            ...
            keys=["head_camera", "twist_camera", "state", "action"],
        )
    ...
    def get_normalizer(self, mode="limits", **kwargs):
        ...
        normalizer["twist_cam"] = get_image_range_normalizer()
        return normalizer
    ...
    def _sample_to_data(self, sample):
        ...
        twist_cam = np.moveaxis(sample["twist_camera"], -1, 1) / 255.0
        data = {
            "obs": {
                "head_cam": head_cam,  # T, 3, H, W
                "twist_cam": twist_cam,  # T, 3, H, W
    ...
    def postprocess(self, samples, device):
        ...
        twist_cam = samples["twist_camera"].to(device, non_blocking=True) / 255.0
        action = samples["action"].to(device, non_blocking=True)
        return {
            "obs": {
                "head_cam": head_cam,  # B, T, 3, H, W
                "twist_cam": twist_cam,  # B, T, 3, H, W
