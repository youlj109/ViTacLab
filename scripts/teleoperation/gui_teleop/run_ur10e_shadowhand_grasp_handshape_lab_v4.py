#!/usr/bin/env python3
"""Standalone wrist+handshape lab v4.

Workflow:
- Wrist control is always marker-driven (same as v1).
- Finger control is upgraded: cyan cube closure + 24-joint offset micro-tuning are blended.
- Optional pickup helper can pre-place marker above object at start.

Hotkeys:
- [f] lock/unlock hand pose (freeze current final 24D hand)
- [p] print current hand_joint_pos_shadow_order
- [s] save YAML snapshot, [g] skip approach, [q] quit
"""
...
_DEFAULT_CLOSED_24: list[float] = [
    0.0, 0.0, 0.0, 0.7909020471572877, 0.7155780426661175, 0.4896060291926067,
    0.0, 0.7909020471572877, 0.7155780426661175, 0.4896060291926067,
    0.0, 0.7909020471572877, 0.7155780426661175, 0.4896060291926067,
    0.07532400449117027, 0.0, 0.6628512395222983, 0.587527235031128, 0.4142820247014365,
    0.037662002245585136, 0.286231217066447, 0.15064800898234054, 0.3615552215576172, 0.7155780426661175,
]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Handshape lab v4: full_ik-style pickup approach + closure cube + 24D micro-tuning.")
    ...
    parser.add_argument("--pickup-ik-yaml", type=str, default="")
    parser.add_argument("--skip-auto-approach", action="store_true")
    parser.add_argument("--approach-max-steps", type=int, default=400)
    parser.add_argument("--approach-hold", type=int, default=12)
    parser.add_argument("--approach-wrist-tol", type=float, default=0.012)
    ...
    parser.add_argument("--closed-hand-yaml", type=str, default="")
    parser.add_argument("--slider-offset-limit", type=float, default=0.5)
    ...
    hand_gui_window = omni.ui.Window("Shadow Hand Offsets v4 (rad)", width=520, height=720, visible=True, dock_preference=omni.ui.DockPreference.RIGHT_TOP)
    ...
    if use_pickup_approach:
        print(f"[INFO] pickup approach cfg: {pickup_ik_path if pickup_ik_path.is_file() else 'built-in defaults'}")

    # Optional one-time helper: place marker above object in full_ik pickup geometry.
    if use_pickup_approach and arm_xf is not None and hasattr(env, "object"):
        ...
        print("[INFO] pickup helper: marker initialized above object; you can move marker anytime.")
        ...
        while simulation_app.is_running():
            ...
            if key:
                ...
                if k == "g" and use_pickup_approach:
                    ...
                    print("[INFO] pickup helper skipped/synced; marker keeps full manual control.")
                if k == "p":
