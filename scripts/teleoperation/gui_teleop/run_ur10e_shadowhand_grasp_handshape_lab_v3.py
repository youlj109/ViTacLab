#!/usr/bin/env python3
"""Hand-shape lab **v3**: same behavior as v2, with finer tuning UX.

**v3 extras** (v2 keeps a simpler UI):

- ``--slider-subdivisions`` — slider step = joint range / N (default 2000).
- Per-joint **FloatField** next to the slider for exact radians.
- ``--load-hand-yaml`` — preload ``hand_joint_pos_shadow_order`` or ``hand_joint_pos_named`` from a YAML saved via ``--save-yaml``.

**Pickup workflow** (matches ``full_ik`` train geometry; see v2 doc for details):

1. **Approach** — Auto arm + open hand to full_ik palm frame (``--pickup-ik-yaml``).
2. **Hand tune** — Arm frozen; 24 sliders + fields. Magenta cube follows wrist (no IK).
3. **Wrist verify** — ``w``: cube drives wrist IK; ``b`` back to hand tune.

Hotkeys: ``w`` / ``b``, ``f`` lock sliders, ``p`` print hand24, ``s`` save, ``g`` skip approach, ``q`` quit.
Use ``[f]`` to lock other joints while tuning one finger; if barely reachable, ``w`` then nudge marker, ``b`` for tweaks.
"""
...
def _load_hand_init_from_yaml(path: Path, sh_names: list[str]) -> np.ndarray:
    """Load 24-dim hand from lab-saved YAML (caller must ensure path.is_file())."""
    data = yaml.safe_load(path.read_text()) or {}
    out = np.zeros(24, dtype=np.float64)
    if "hand_joint_pos_shadow_order" in data and isinstance(data["hand_joint_pos_shadow_order"], list):
        lst = data["hand_joint_pos_shadow_order"]
        for i in range(min(24, len(lst))):
            out[i] = float(lst[i])
        return out
    if "hand_joint_pos_named" in data and isinstance(data["hand_joint_pos_named"], dict):
        named = data["hand_joint_pos_named"]
        for i, nm in enumerate(sh_names):
            if nm in named:
                out[i] = float(named[nm])
        return out
    print(
        "[WARN] YAML has no hand_joint_pos_shadow_order or hand_joint_pos_named; "
        "sliders start at zero."
    )
    return out
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Hand-shape lab v3: v2 workflow + finer sliders, FloatField, optional --load-hand-yaml."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="YAML from --save-yaml: preload hand_joint_pos_shadow_order or hand_joint_pos_named into sliders.",
    )
    parser.add_argument(
        "--slider-subdivisions",
        type=int,
        default=2000,
        help="FloatSlider step = (joint_max-joint_min)/N; larger N = finer adjustment (default 2000).",
    )
    ...
    load_hand_path: Path | None = None
    load_h_arg = str(args.load_hand_yaml).strip()
    if load_h_arg:
        load_hand_path = Path(load_h_arg).expanduser()
        if not load_hand_path.is_absolute():
            load_hand_path = (repo_root / load_hand_path).resolve()
    ...
    if load_hand_path is not None:
        if not load_hand_path.is_file():
            print(f"[WARN] --load-hand-yaml not found: {load_hand_path}")
        else:
            init_h = _load_hand_init_from_yaml(load_hand_path, sh_names)
            print(f"[INFO] Hand slider init from {load_hand_path}")
    ...
    _subdiv = max(50, int(args.slider_subdivisions))
    hand_gui_window = omni.ui.Window("Shadow Hand Joints v3 (rad)", width=520, height=720, visible=True, dock_preference=omni.ui.DockPreference.RIGHT_TOP)
    with hand_gui_window.frame:
        with omni.ui.ScrollingFrame(horizontal_scrollbar_policy=omni.ui.ScrollBarPolicy.SCROLLBAR_AS_NEEDED):
            with omni.ui.VStack(spacing=3, height=0):
                ...
                omni.ui.Label(
                    f"Slider step ≈ range/{_subdiv} (--slider-subdivisions). [f] lock while tweaking one finger.",
                    word_wrap=True,
                )
