...
Loads the same task presets as ``run_ur10e_shadowhand_arm_pose_from_marker.py``. Spawns
``/World/Debug/GraspClosureTarget`` (cyan). Move it along ``--closure-axis``; its world
coordinate in ``[closure_min, closure_max]`` maps to grasp alpha in ``[0, 1]``. Thumb joints
(THJ*) stay at 0 for a four-finger cup-style preset; finger flexion interpolates from open
to a reference closed pose (tunable via ``--closed-hand-yaml``).
...
Examples (Isaac Sim python)::
    ...
    ./python.sh scripts/teleoperation/gui_teleop/run_ur10e_shadowhand_grasp_pose_from_marker.py \\
        --task pour --enable_cameras \\
        --arm-yaml scripts/teleoperation/gui_teleop/config/pregrasp_arm_pour.yaml \\
        --save-yaml scripts/teleoperation/gui_teleop/config/recorded_grasp.yaml
"""
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        type=str,
        required=True,
        help="Freeze arm to joint_pos or arm_joint_pos from this YAML (repo-relative or absolute).",
    )
    p.add_argument(
        "--closed-hand-yaml",
        type=str,
    ...
    p.add_argument("--object-to-palm-offset", type=float, nargs=3, default=(0.0, 0.05, -0.03))
    p.add_argument("--palm-in-wrist-pos", type=float, nargs=3, default=(0.0, 0.0, 0.35))
    p.add_argument(
        "--palm-in-wrist-euler",
        type=float,
        nargs=3,
    ...
    p.add_argument("--palm-orient", choices=("fixed", "pickup_down"), default="pickup_down")
    p.add_argument("--palm-euler", type=float, nargs=3, default=(0.0, 2.2, 0.0))
    p.add_argument("--palm-normal-local", type=float, nargs=3, default=(0.0, 0.0, 1.0))
    p.add_argument("--world-down", type=float, nargs=3, default=(0.0, 0.0, -1.0))
    p.add_argument("--palm-yaw-offset", type=float, default=3.141592653589793)
    p.add_argument(
        "--sync-full-ik-config",
        type=str,
        default="",
    ...
    p.add_argument(
        "--disable-hotkeys",
        action="store_true",
        help="Disable terminal hotkeys. By default: [f]=toggle finger lock, [r]=manual reset, [q]=quit.",
    )
    p.add_argument(
        "--manual-reset-only",
        action="store_true",
...
def _install_terminal_cbreak() -> tuple[int, list[int]] | None:
    if os.name != "posix":
        return None
...
def _restore_terminal(state: tuple[int, list[int]] | None) -> None:
    if state is None:
        return
    fd, old = state
...
def main() -> int:
    ...
    elif str(args.arm_control) in ("cup_relative", "ik_trajectory"):
        if not hasattr(env, "cup"):
            raise RuntimeError("arm-control=cup_relative/ik_trajectory requires env.cup (e.g. pour task).")
        arm_ik = VideoTeleopControl()
    ...
    print(f"[INFO] Arm source YAML: {arm_path}")
    if str(args.arm_control) == "fixed":
        ...
        print(
            "[INFO] Arm mode=ik_trajectory. Wrist follows ik_rl-style cup anchor + object_to_palm_offset; "
            "this matches train-time arm approach semantics."
        )
    else:
        print(
    ...
    if not bool(args.disable_hotkeys):
        print("[INFO] Hotkeys: [f] toggle finger lock, [r] reset env, [q] quit.")
