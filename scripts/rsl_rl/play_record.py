...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    print(f"[INFO] Starting simulation. Target: {args_cli.num_episodes} successful trajectories.")
    ...
    def _is_success(infos):
        s = infos.get("logs_rew_curr_success", 0)
        if hasattr(s, "item"):
            s = s.item()
        return float(s) >= 0.5
    ...
    while True:
        ...
        truncated = max_steps_per_episode is not None and max_steps_per_episode > 0 and step_in_episode >= max_steps_per_episode
        success_this_step = args_cli.save_data and _is_success(infos)
        ...
        if args_cli.save_data:
            if success_this_step:
                ...
                for k, v in current_ep_buffers.items():
                    if k == "camera_rgb":
                        for cam_key, cam_buf in v.items():
                            if len(cam_buf) > 0:
                                ep_data_torch[cam_key] = torch.stack(cam_buf, dim=0)
                    elif len(v) > 0:
                        ep_data_torch[k] = torch.stack(v, dim=0)
                all_episodes_data.append(ep_data_torch)
                episodes_collected += 1
                print(f"[INFO] Success at step {step_in_episode}; saved episode {episodes_collected} / {args_cli.num_episodes}")
                current_ep_buffers = init_episode_buffers(args_cli)
                step_in_episode = 0
                with torch.inference_mode():
                    obs, _ = env.reset()
                ...
                # 达到 max_steps 仍未成功：丢弃轨迹，不保存，开始下一条
                print(f"[INFO] Discarded trajectory (max_steps={max_steps_per_episode} without success).")
                current_ep_buffers = init_episode_buffers(args_cli)
                step_in_episode = 0
                with torch.inference_mode():
                    obs, _ = env.reset()
            elif env_done:
                # 环境结束（如超时）但未成功：丢弃轨迹，不保存；env 已内部 reset，obs 已是新 episode
                current_ep_buffers = init_episode_buffers(args_cli)
                step_in_episode = 0
