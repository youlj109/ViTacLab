...
"""Play / eval a **hand-only** RSL-RL policy with GPU differential IK (dual-arm; same stack as ``train_ik_rl_dual.py``).

Examples::

    ./isaaclab.sh -p scripts/rsl_rl/ik_rl/play_ik_rl_dual.py \\
        --task Isaac-UR10e-Dual-Shadow-Hand-UnscrewBottleCap-Direct-v0 \\
        --num_envs 64 --checkpoint /path/to/model.pt

IK / EE waypoint YAML should match training; defaults mirror ``train_ik_rl_dual.py`` / ``ik_rl_unscrew_dual.yaml``.
...
parser = argparse.ArgumentParser(description="Play hand-only RSL-RL policy with GPU IK (dual-arm MARL → single).")
...
# IK (YAML lists); optional overrides
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="YAML with task + trajectories (see configs/ik_rl_unscrew_dual.yaml). Omitted: auto-load. 'none' = off.",
)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    print(
        f"[INFO] IK play (dual): hand actions={expander.num_hand}, full actuated={expander.num_actuated}; "
...
if __name__ == "__main__":
    if not args_cli.task:
        raise SystemExit("--task is required (e.g. Isaac-UR10e-Dual-Shadow-Hand-UnscrewBottleCap-Direct-v0).")
    main()
