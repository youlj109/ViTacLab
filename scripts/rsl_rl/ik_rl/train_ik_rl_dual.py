...
"""Train dual Shadow **hands** with RSL-RL; each UR10e **arm** follows GPU differential IK + EE waypoints.

Same stack as ``train_ik_rl_single.py`` but uses :class:`DualArmIkHandActionExpander` after
``multi_agent_to_single_agent`` (policy = concatenated hand actions for right and left).

Each arm uses a list of world-frame poses for ``ee_body`` (default ``wrist_3_link``): ``pos``, ``quat`` (wxyz),
``steps``. Optional per-arm lists: ``trajectory_right`` / ``trajectory_left`` in YAML; if omitted, ``trajectory``
is copied to both.

If the env randomizes object/bottle position, add the same offset to EE targets via ``env.ik_rl_trajectory_xyz_offset``.

Example::

    ./isaaclab.sh -p scripts/rsl_rl/ik_rl/train_ik_rl_dual.py \\
        --task Isaac-UR10e-Dual-Shadow-Hand-UnscrewBottleCap-Direct-v0 --num_envs 16

Defaults load from ``configs/ik_rl_unscrew_dual.yaml`` when present (``--ik-config`` / ``none`` same as single).
"""
...
parser = argparse.ArgumentParser(description="Train hand-only RL with GPU differential IK (dual-arm MARL → single).")
...
# IK: EE waypoints loaded from YAML (lists); optional overrides below
parser.add_argument("--trajectory", default=None, help=argparse.SUPPRESS)
parser.add_argument("--trajectory_right", default=None, dest="trajectory_right", help=argparse.SUPPRESS)
parser.add_argument("--trajectory_left", default=None, dest="trajectory_left", help=argparse.SUPPRESS)
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="YAML with task + trajectory lists (see configs/ik_rl_unscrew_dual.yaml). "
...
from ik_rl_load_config import (
    ...
    default_unscrew_dual_ik_yaml_path,
    resolve_ik_config_path,
    warn_if_task_mismatch_with_ik_yaml,
)

apply_sys_argv_ik_yaml_defaults(parser, default_unscrew_dual_ik_yaml_path())
args_cli, hydra_args = parser.parse_known_args()
_cfg_path = resolve_ik_config_path(sys.argv, default_unscrew_dual_ik_yaml_path())
...
from ik_rl_hand_vec_env import DualArmIkHandActionExpander, IkHandRslRlVecEnvWrapper, build_ik_cfg_from_trajectory_args
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    """Train hand-only policy with GPU differential IK (dual arms)."""
    ...
    if args_cli.max_episode_length is not None:
        ...
        if hasattr(env_cfg, "max_episode_length"):
            ...
            logger.info(
                "[train_ik_rl_dual] --max_episode_length=%d → episode_length_s=%.6f (step_dt=%.6f)",
                n,
            ...
            logger.warning(
                "[train_ik_rl_dual] --max_episode_length ignored: env_cfg has no usable max_episode_length / "
    ...
    ik_cfg_right = build_ik_cfg_from_trajectory_args(args_cli, arm="right")
    ik_cfg_left = build_ik_cfg_from_trajectory_args(args_cli, arm="left")
    expander = DualArmIkHandActionExpander(base, ik_cfg_right, ik_cfg_left)
    print(
        f"[INFO] Hand-only RL (dual): policy actions={expander.num_hand}, full actuated={expander.num_actuated}; "
        f"EE waypoints right={len(ik_cfg_right.waypoints)} left={len(ik_cfg_left.waypoints)}"
    )
    ...
    dump_yaml(
        os.path.join(log_dir, "params", "ik_rl_hand.yaml"),
        {
            ...
            "trajectory_right": getattr(args_cli, "trajectory_right", None) or args_cli.trajectory,
            "trajectory_left": getattr(args_cli, "trajectory_left", None) or args_cli.trajectory,
            "ee_body_name": ik_cfg_right.ee_body_name,
            "ik_method": ik_cfg_right.ik_method,
            "ik_lambda": ik_cfg_right.ik_lambda,
            "ik_k_val": ik_cfg_right.ik_k_val,
            "ik_delta_scale": ik_cfg_right.ik_delta_scale,
