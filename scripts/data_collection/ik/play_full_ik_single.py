...
Examples::
    ...
    ./isaaclab.sh -p scripts/data_collection/ik/play_full_ik_single.py --task Isaac-UR10eShadowHand-Pickup-Direct-v0 \\
...
_COMMON_DIR = os.path.join(_REPO_ROOT, "scripts", "common")
if _COMMON_DIR not in sys.path:
    sys.path.insert(0, _COMMON_DIR)

_DEFAULT_FULL_IK_YAML = Path(_THIS_DIR) / "configs" / "full_ik" / "full_ik_pour.yaml"
...
parser.add_argument(
    ...
    help="YAML with phase_schedule + palm/IK (see data_collection/ik/configs/full_ik/full_ik_pour.yaml).",
)
...
from rl.full_ik_load_config import (
    IK_YAML_KEYS,
    _coerce,
...
# Tactile viewer needs scene cameras / sensors (match scripts/data_collection/manual/record_observations.py).
...
from rl.full_ik_arm_ik_expander import IkRlHandArmCfg, parse_trajectory_phases
from rl.full_ik_hand_vec_env import PhasedArmIkHandExpander, PhasedIkHandRslRlVecEnvWrapper
...
def _save_play_episode_npz(out_dir: str, fname: str, bufs: dict):
    """Write one episode record_dict buffers to ``fname`` under ``out_dir``."""
    if not bufs:
        return None, 0
    first_key = next(iter(bufs.keys()))
    if len(bufs[first_key]) == 0:
        return None, 0
    T = len(bufs[first_key])
    path = os.path.join(out_dir, fname)
    save_kwargs = {}
    for k, seq in bufs.items():
        if len(seq) == 0:
            continue
        save_kwargs[k] = np.stack(seq, axis=0)
...
def _clear_record_buffers(bufs: dict) -> None:
    for k in bufs:
        bufs[k].clear()


def _extract_policy_obs_row(obs: object, env_index: int) -> np.ndarray | None:
    """Extract one-env policy observation row from common wrapper outputs."""
    pol = None
    # Prefer direct key access first: some wrappers return custom mapping-like objects.
    try:
        pol = obs["policy"]  # type: ignore[index]
    except Exception:
        pol = None
    if pol is None:
        if isinstance(obs, dict):
            ...
            first = obs[0]
            try:
                pol = first["policy"]  # type: ignore[index]
            except Exception:
                if isinstance(first, dict):
                    pol = first.get("policy", None)
                elif torch.is_tensor(first):
                    pol = first
    if pol is None:
        return None
    if torch.is_tensor(pol):
        pol_t = cast(torch.Tensor, pol)
        if pol_t.ndim < 2:
            return None
        ei = max(0, min(int(env_index), int(pol_t.shape[0]) - 1))
        return pol_t[ei].detach().cpu().numpy()
    if isinstance(pol, np.ndarray):
        pol_np = cast(np.ndarray, pol)
        if pol_np.ndim < 2:
            return None
        ei = max(0, min(int(env_index), int(pol_np.shape[0]) - 1))
        return np.asarray(pol_np[ei]).copy()
    # Last fallback for array-like objects.
    try:
        pol_np = np.asarray(pol)
        if pol_np.ndim < 2:
            return None
        ei = max(0, min(int(env_index), int(pol_np.shape[0]) - 1))
        return np.asarray(pol_np[ei]).copy()
...
def _extract_record_row(obs: object, env_index: int) -> dict[str, np.ndarray] | None:
    """Extract one-env row from observation record dict."""
    record = None
    try:
    ...
    for k, v in record.items():
        if torch.is_tensor(v):
            if v.ndim == 0:
                row[k] = v.detach().cpu().numpy()
            else:
                ei = max(0, min(int(env_index), int(v.shape[0]) - 1))
                row[k] = v[ei].detach().cpu().numpy()
    return row if row else None


def _extract_record_row_from_env(env_unwrapped: object, env_index: int) -> dict[str, np.ndarray] | None:
    """Fallback path for tasks whose policy observation omits ``record``.

    Walk the common wrapper chain and prefer the maintained
    ``_build_record_dict`` contract.  This is required by Pour: its compact
    policy observation intentionally contains no dense camera/tactile record,
    while the shared UR10e+ShadowHand base still provides the complete
    ...
    for _ in range(20):
        ...
        if callable(build_record):
            ...
            if built_row is not None:
                # A task-specific obs['record'] may contain fields that extend
                # or override the shared base schema.
                if observation_row is not None:
                    built_row.update(observation_row)
                return built_row
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    if bool(getattr(args_cli, "record_data", False)):
        _enable_cams = True
    ...
    if not FULL_IK_PHASE_SCHEDULE:
        raise RuntimeError(
            ...
            "(see scripts/data_collection/ik/configs/full_ik/full_ik_pour.yaml)."
        )
    ...
    if args_cli.record_data:
        ...
        if wrapped.num_envs > 1:
            print(
                "[WARN] --record_data with success-only saving is most reliable with --num_envs 1. "
                "When num_envs > 1, episode success attribution to record_env_index can be ambiguous."
            )
    ...
    prev_success_stats: tuple[int, int, float] | None = None
    if hasattr(wrapped.unwrapped, "get_episode_success_stats"):
        prev_success_stats = wrapped.unwrapped.get_episode_success_stats()

    while simulation_app.is_running():
        t0 = time.time() if show_tactile else None
        step += 1
        record_row = None
        if args_cli.record_data and rec_bufs is not None:
            record_row = _extract_record_row(obs, record_ei)
            if record_row is None:
                record_row = _extract_record_row_from_env(wrapped.unwrapped, record_ei)
        ...
        if args_cli.record_data and rec_bufs is not None and record_row is not None:
            if not rec_bufs:
                for k in record_row:
                    rec_bufs[k] = []
                print(
                    "[INFO] initialized canonical record buffers: "
                    + ", ".join(sorted(record_row.keys()))
                )
            for k, v in record_row.items():
                rec_bufs.setdefault(k, []).append(v)

        # Some wrappers may not expose done=True reliably on play; use episode success counters
        # from env resets as the robust episode boundary signal.
        if args_cli.record_data and rec_bufs is not None:
            # Primary criterion (example/play style): per-step success signal from env extras.
            if isinstance(_extras, dict) and "curr_success_per_env" in _extras:
                succ = _extras["curr_success_per_env"]
                try:
                    succ_i = bool(succ[record_ei].item())
                except Exception:
                    succ_i = False
                if succ_i:
                    ...
                    if path is not None:
                        print(f"[INFO] recorded successful episode {path} (T={T})")
                        rec_ep_idx += 1
                        if args_cli.record_max_episodes > 0 and rec_ep_idx >= args_cli.record_max_episodes:
                            break
                    _clear_record_buffers(rec_bufs)
                    continue

            # Fallback criterion: episode boundary from cumulative success stats.
            ue = wrapped.unwrapped
            if hasattr(ue, "get_episode_success_stats"):
                cur_stats = ue.get_episode_success_stats()
                if prev_success_stats is not None:
                    prev_ok, prev_total, _ = prev_success_stats
                    cur_ok, cur_total, _ = cur_stats
                    if cur_total > prev_total:
                        episode_success = cur_ok > prev_ok
                        print(
                            "[INFO] episode boundary detected: "
                            f"prev=({prev_ok}/{prev_total}) -> cur=({cur_ok}/{cur_total}), "
                            f"success={episode_success}, buffered_steps={len(next(iter(rec_bufs.values()))) if rec_bufs else 0}"
                        )
                        if episode_success:
                            assert record_dir is not None
                            path, T = _save_play_episode_npz(record_dir, f"episode_{rec_ep_idx:04d}.npz", rec_bufs)
                            if path is not None:
                                ...
                                print(
                                    "[WARN] successful episode detected but no trajectory samples were buffered; "
                                    "check observation extraction / wrapper output format."
                                )
                        else:
                            t_drop = len(next(iter(rec_bufs.values()))) if rec_bufs else 0
                            _clear_record_buffers(rec_bufs)
                            print(
                                f"[INFO] dropped failed episode #{rec_drop_idx:04d} "
                                f"(record_env_index={record_ei}, T={t_drop})"
                            )
                            rec_drop_idx += 1
                prev_success_stats = cur_stats
    ...
    if args_cli.record_data and rec_bufs is not None and rec_bufs and len(next(iter(rec_bufs.values()))) > 0:
        print(
            "[INFO] play ended mid-episode; unsaved partial trajectory is discarded by design "
