...
Loads the same task presets as ``scripts/data_collection/manual/record_observations.py``. Spawns a small
...
Examples (Isaac Sim python):
    ...
    ./python.sh scripts/data_collection/manual/record_arm_pose.py \\
    ...
    ./python.sh scripts/data_collection/full_trajectory/record_single.py \\
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="UR10e arm IK from a visual marker (same task presets as record_observations.py).",
    )
    p.add_argument(
        "--task",
        type=str,
        default="pickup",
        help=(
            "Single-UR10e preset from task_entries_single.py or any registered Gymnasium id with "
            "env_cfg_entry_point."
        ),
    )
...
def main() -> int:
    ...
    hand_pose_preset_path = (
        (_repo_root() / "scripts/data_collection/full_trajectory/hand_pose_presets")
        / f"{EnvCls.__name__}__{CfgCls.__name__}.json"
    )
    pose_keyframe_path = (
        (_repo_root() / "scripts/data_collection/full_trajectory/pose_keyframes")
        / f"{EnvCls.__name__}__{CfgCls.__name__}.json"
    )
                ...
                with omni.ui.ScrollingFrame(horizontal_scrollbar_policy=omni.ui.ScrollBarPolicy.SCROLLBAR_AS_NEEDED):
                    with omni.ui.VStack(spacing=3, height=0):
                        ...
                        with omni.ui.HStack(spacing=4):
                            ...
                            record_path_model.set_value("./scripts/data_collection/full_trajectory/records")
