#!/usr/bin/env python3
"""Roll out a Diffusion Policy checkpoint on forge dexhand tasks."""
...
def _policy_observation_from_record(obs: object, num_envs: int) -> list[dict[str, torch.Tensor]]:
    ...
    record = obs["record"]
    if "third_person_camera" not in record:
        raise RuntimeError("obs['record'] has no 'third_person_camera'. Pass --enable_cameras.")
    if "joint_pos" not in record:
        raise RuntimeError("obs['record'] has no 'joint_pos'.")
    ...
    for env_i in range(num_envs):
        ...
        if "twist_camera" in record:
            item["twist_camera"] = _tensor_to_policy_value(record["twist_camera"], env_i)
        out.append(item)
    return out
...
def _joint_pos_to_normalized_action(env: Any, joint_pos: np.ndarray) -> np.ndarray:
    ...
    if action.shape[-1] != int(env.num_actions):
        raise ValueError(
            f"Expected DP joint_pos output dim {env.num_actions}, got {action.shape[-1]}."
        )
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--task_name", type=str, default="", help="DP checkpoint task name; default uses the Gym id.")
    parser.add_argument("--data_num", type=int, required=True, help="DP data count used in checkpoint directory name.")
    parser.add_argument("--checkpoint_num", type=int, required=True, help="DP checkpoint number without .ckpt.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="joint_pos",
        help=(
            "How to interpret DP's 30D output. Use 'joint_pos' for checkpoints trained "
...
def main() -> int:
    ...
    for path in (repo_root, repo_root / "source"):
    ...
    from policy.Diffusion_Policy.deploy_policy import Encapsulation
