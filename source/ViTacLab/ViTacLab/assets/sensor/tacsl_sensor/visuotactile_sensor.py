...
class VisuoTactileSensor(SensorBase):
    ...
    **Current Limitations:**
    ...
    def _initialize_camera_tactile(self):
        ...
        self._data.tactile_rgb_image_corrected = torch.zeros(
            (self._num_envs, self.cfg.camera_cfg.height, self.cfg.camera_cfg.width, 3), device=self._device, dtype=torch.uint8
        )
    ...
    def _find_contact_object_components(self) -> tuple[Any, Any]:
        ...
        if contact_object_mesh is None:
            # Fallback: accept the first collision mesh even if its approximation
            # is not explicitly marked as "sdf". This is useful for custom assets
            # (e.g., DexCube variants) where the collision approximation flag may
            # not be set but an SDF view can still be constructed.
            def is_collision_mesh(prim: Usd.Prim) -> bool:
                return prim.HasAPI(UsdPhysics.MeshCollisionAPI)
            
            contact_object_mesh = sim_utils.get_first_matching_child_prim(
                contact_object_prim.GetPath(), predicate=is_collision_mesh
            )
            if contact_object_mesh is None:
                raise RuntimeError(
                    f"No SDF mesh or collision mesh found under contact object at path: "
                    f"{contact_object_prim.GetPath().pathString}"
                )
            logger.warning(
                "No SDF mesh found under contact object at path %s; "
                "falling back to first collision mesh %s for TacSL force field.",
                contact_object_prim.GetPath().pathString,
                contact_object_mesh.GetPath().pathString,
            )
    ...
    def _initialize_force_field_buffers(self):
        ...
        self._data.tactile_shear_magnitude = torch.zeros((self._num_envs, num_pts), device=self._device)
        self._data.tactile_tangential_speed = torch.zeros((self._num_envs, num_pts), device=self._device)
        self._data.contact_mask = torch.zeros((self._num_envs, num_pts), device=self._device, dtype=torch.bool)
        self._data.slip_mask = torch.zeros((self._num_envs, num_pts), device=self._device, dtype=torch.bool)
    ...
    def _update_buffers_impl(self, env_ids: Sequence[int]):
        ...
        # Update unified tri-modal interface after modality updates.
        self._update_tri_modal_output()

    def _update_tri_modal_output(self) -> None:
        """Build a unified tri-modal output view for downstream policy code."""
        shear_force = self._data.tactile_shear_force
        if shear_force is not None:
            self._data.tactile_shear_magnitude = torch.norm(shear_force, dim=-1)

        pen = self._data.penetration_depth
        if pen is not None:
            contact = pen > 0.0
            self._data.contact_mask = contact
        else:
            contact = None

        if self._data.tactile_tangential_speed is not None:
            slip_from_speed = self._data.tactile_tangential_speed > float(self.cfg.slip_speed_threshold)
            self._data.slip_mask = slip_from_speed if contact is None else (slip_from_speed & contact)
        elif self._data.tactile_shear_magnitude is not None and self._data.tactile_normal_force is not None:
            # Conservative fallback for legacy SDF path without explicit tangential speed.
            mu = float(max(self.cfg.friction_coefficient, 1e-12))
            slip_from_force = self._data.tactile_shear_magnitude > (mu * torch.clamp(self._data.tactile_normal_force, min=0.0))
            self._data.slip_mask = slip_from_force if contact is None else (slip_from_force & contact)

        self._data.tri_modal = {
            "vision_rgb": self._data.tactile_rgb_image,
            "vision_rgb_corrected": self._data.tactile_rgb_image_corrected,
            "vision_depth": self._data.tactile_depth_image,
            "vision_height_corrected": self._data.tactile_height_map_corrected,
            "force_normal": self._data.tactile_normal_force,
            "force_shear": self._data.tactile_shear_force,
            "force_shear_magnitude": self._data.tactile_shear_magnitude,
        ...
        self._data.policy_obs = None
        if (
            self._data.tactile_normal_force is not None
            and self._data.tactile_shear_force is not None
            and self._data.tactile_tangential_speed is not None
            and self._data.contact_mask is not None
            and self._data.slip_mask is not None
        ):
            fn = self._data.tactile_normal_force.unsqueeze(-1)
            fs = self._data.tactile_shear_force
            vt = self._data.tactile_tangential_speed.unsqueeze(-1)
            cm = self._data.contact_mask.float().unsqueeze(-1)
            sm = self._data.slip_mask.float().unsqueeze(-1)
            force_token = torch.cat([fn, fs, vt, cm, sm], dim=-1)
            policy_obs: dict[str, torch.Tensor] = {
                "force_token": force_token,
                "force_flat": force_token.reshape(force_token.shape[0], -1),
            }
            if self._data.tactile_depth_image is not None:
                policy_obs["depth_flat"] = self._data.tactile_depth_image.reshape(self._data.tactile_depth_image.shape[0], -1)
            if self._data.tactile_rgb_image is not None:
                policy_obs["rgb_flat"] = self._data.tactile_rgb_image.reshape(self._data.tactile_rgb_image.shape[0], -1)
            self._data.policy_obs = policy_obs
