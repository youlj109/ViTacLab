"""UR10e + ShadowHand dual-arm articulation configs (left + right USD roots).

Layout: two bases separated by 1 m along :math:`-Y`, with the left root rotated 180° about
:math:`Z` so both palms face the shared workspace between them.
"""
...
UR10E_DUAL_SHADOWHAND_LEFT_CFG: ArticulationCfg = ArticulationCfg(
    ...
    init_state=ArticulationCfg.InitialStateCfg(
        pos=(0.6, 1.15, 0.5),
        rot=(0.5, -0.5, 0.5, -0.5),
        joint_pos=UR10E_DUAL_SHADOWHAND_DEFAULT_ARM_JOINT_POS,
    ),
    actuators=_SHARED_ACTUATORS,
    soft_joint_pos_limit_factor=1.0,
)

UR10E_DUAL_SHADOWHAND_RIGHT_CFG: ArticulationCfg = ArticulationCfg(
    prim_path="/World/envs/env_.*/RightRobot",
    spawn=sim_utils.UsdFileCfg(
        usd_path="source/ViTacLab/ViTacLab/assets/data/Robots/ShadowHand/ur10e/ur10e_shadow_right_hand_glb_withtac.usd",
        **_SHARED_USD_KWARGS,
    ),
    init_state=ArticulationCfg.InitialStateCfg(
        pos=(0.5, 1.15, 0.5),
...
@configclass
class UR10eDualShadowHandDirectSceneCfg(InteractiveSceneCfg):
    """Dual-arm scene container (cameras / TacSL added in env after clone)."""


def build_ur10e_dual_shadowhand_third_person_camera_cfg() -> TiledCameraCfg:
    """Third-person camera centered between the two arm bases (Y from 0 to -1 m)."""
...
def build_ur10e_dual_shadowhand_tacsl_sensor_cfgs(
    scene_cfg: UR10eDualShadowHandTacSLSceneCfg,
    *,
    robot_prim_name: str,
) -> dict[str, VisuoTactileSensorCfg]:
    """TacSL GelSight cfgs for one UR10e+Shadow articulation (``LeftRobot`` or ``RightRobot``)."""
