"""Shadow Hand Over environment implementation.
...
class ShadowHandOverEnv(DirectMARLEnv):
    ...
    def _setup_scene(self):
        self._expected_tactile_sensor_names = (
        ...
        if getattr(self.cfg, "enable_cameras", False):
            if getattr(self.cfg, "enable_third_person_camera", True):
            ...
            for prefix, robot_prim_name in (("right_", "RightRobot"), ("left_", "LeftRobot")):
                sensor_cfgs = build_shadow_hand_tacsl_sensor_cfgs(
                    robot_prim_name=robot_prim_name,
    ...
    def _build_record_dict(self) -> dict[str, torch.Tensor]:
        """Return 10-channel tactile pose/force/RGB without changing MARL observations."""
        record = build_shadow_hand_tactile_record(
            self,
            (
                (self.right_hand, shadow_hand_tacsl_sensor_keys("right_")),
                (self.left_hand, shadow_hand_tacsl_sensor_keys("left_")),
            ),
        )
        if "third_person_camera" in self.scene.sensors:
            ...
            cam_quat = getattr(cam.data, "quat_w_ros", None)
            if cam_quat is None:
                cam_quat = getattr(cam.data, "quat_w", None)
            if cam_quat is not None:
                cam_pos_env = cam.data.pos_w - self.scene.env_origins
                record["third_person_camera_pos"] = (
                    torch.cat((cam_pos_env, cam_quat), dim=-1).unsqueeze(1).detach().cpu()
                )
        return record
    ...
    def _get_rewards(self) -> dict[str, torch.Tensor]:
        ...
        within_tol = goal_dist <= float(self.cfg.success_pos_tol)
        self._success_hold_counter = torch.where(
            within_tol,
            self._success_hold_counter + 1,
            torch.zeros_like(self._success_hold_counter),
        )
        success_mask = self._success_hold_counter >= int(self.cfg.success_hold_steps)
