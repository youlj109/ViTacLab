"""Shadow Hand Over environment configuration.
...
# ViTacLab Shadow Hand with tactile shells (same as shadow_hand_env_cfg)
...
class ShadowHandOverEnvCfg(DirectMARLEnvCfg):
    """Two-hand object transfer with optional 10-channel TacSL recording.

    ``enable_cameras=True`` creates five real GelSight sensors per hand.  The
    MARL observation/state dimensions remain unchanged; tactile pose, force,
    and RGB are available through the canonical record builder.
    """
    ...
    third_person_camera_pos: tuple[float, float, float] = (0.45, -0.55, 1.35)
    third_person_camera_rot: tuple[float, float, float, float] = (0.88439, 0.46675, -0.001, -0.00191)
    ...
    # robot (shadow_hand_withtac.usd + collision, aligned with shadow_hand_env_cfg)
    ...
    fall_dist = 0.24
    # Successful hand-over means the object remains within this distance of
    # the target for several consecutive control steps. The signal is exposed
    # through extras for RSL-RL data collection and policy validation.
