"""Shared constants for ViTacSim NF/SF validation beta (rebattle figures)."""
...
ADVISOR_MASS_KG = {cid: g / 1000.0 for cid, g in ADVISOR_CASE_MASS_G.items()}
MODES = ("tacsl", "vitacsim")

NF_SCHEMA = "nf_v3_beta"
SF_SCHEMA = "sf_lateral_v2"

# Lower finger root => deeper pad contact => PhysX Fn closer to nominal (was 0.448 @ ~0.82 ratio).
DEFAULT_FINGER_ROOT_Z = 0.444

# Mark SF sweep points invalid when normal load drops below this fraction of mg.
CONTACT_VALID_FN_RATIO = 0.5

# Per-weight lateral Fx sweep caps (N) — avoids pushing light weights off the pad.
...
def nominal_fn_n(case_id: str) -> float:
    if case_id in ADVISOR_MASS_KG:
        return ADVISOR_MASS_KG[case_id] * 9.81
    return WEIGHT_MASS_KG[case_id] * 9.81
...
def resolve_force_render_k_ref(case_id: str, cli_value: float) -> float:
    ...
    return adaptive_force_render_k_ref(case_id)
