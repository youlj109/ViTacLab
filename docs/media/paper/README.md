# Paper Figure Exports

These images were exported from the author-provided ViTacLab manuscript source for use on the repository homepage.

| Repository image | Manuscript source | Processing |
|---|---|---|
| `vitacbench.png` | `imgs/vitaclab-figure3.pdf` | PNG export at 2,200 px width |
| `vitacpolicy.png` | `imgs/vitaclab-figure4.pdf` | PNG export at 2,200 px width |
| `sim-validation.png` | `imgs/vitaclab-sim-validation.png` | Original PNG copied without content edits |

Only the figures currently used by the homepage are included. Numerical labels and scientific claims in these exports should remain synchronized with the manuscript.
